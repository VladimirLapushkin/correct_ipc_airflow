...
module "airflow-cluster" {
  ...
  provider_config    = var.yc_config
  yc_storage_endpoint_url  = var.yc_storage_endpoint_url
...
module "mlflow-server" {
  ...
  subnet_id              = module.network.subnet_id
  service_account_id     = module.iam.service_account_id
  ubuntu_image_id_mlflow = var.ubuntu_image_id_mlflow
...
resource "local_file" "variables_file" {
  content = jsonencode({
    ...
    # S3 ipc/source
    S3_IPC_ACCESS_KEY = var.yc_ipc_bucket_ak
    S3_IPC_SECRET_KEY = var.yc_ipc_bucket_sk
    S3_IPC_PATH       = var.yc_ipc_bucket_path
    ...
    # K8S balancer
    K8S_BALANCER = var.k8s_balancer_ip

    # settings for training
    TRAINING_NUM_MONTH   = var.traininq_num_month
    TRAINING_LAST_MONTH  = var.traininq_last_month
...
resource "null_resource" "update_env" {
  provisioner "local-exec" {
    ...
    set -eu
      ...
      if grep -q "^TRAINING_NUM_MONTH=" ../.env; then
        sed -i "s|^TRAINING_NUM_MONTH=.*|TRAINING_NUM_MONTH=$TRAINING_NUM_MONTH|" ../.env
      else
        echo "TRAINING_NUM_MONTH=$TRAINING_NUM_MONTH" >> ../.env
      fi

      if grep -q "^TRAINING_LAST_MONTH=" ../.env; then
        sed -i "s|^TRAINING_LAST_MONTH=.*|TRAINING_LAST_MONTH=$TRAINING_LAST_MONTH|" ../.env
      else
        echo "TRAINING_LAST_MONTH=$TRAINING_LAST_MONTH" >> ../.env
      fi
      ...
      if grep -q "^K8S_BALANCER=" ../.env; then
        sed -i "s|^K8S_BALANCER=.*|K8S_BALANCER=$K8S_BALANCER|" ../.env
      else
        echo "K8S_BALANCER=$K8S_BALANCER" >> ../.env
      fi
